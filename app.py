...
def get_students():
    ...
    for student in students:
        ...
        status = "expired" if student["expiry_date"] < today else "active"
...
def add_student():
    ...
    # -------------------------------------------------
    # CHECK IF SET IS ALREADY OCCUPIED
    # -------------------------------------------------
    ...
    try:
        ...
        existing = conn.execute("""
            ...
            AND set_number = ?
            AND expiry_date >= ?
        """, (section, set_number, datetime.now().strftime("%Y-%m-%d"))).fetchone()
...
def dashboard():
    ...
    occupied_sets = general_occupied + vip_occupied

    available_sets = total_sets - occupied_sets

    general_available = 106 - general_occupied

    vip_available = 57 - vip_occupied
...
def export_report():
    ...
    try:
        ...
        query += " ORDER BY joining_date ASC, id ASC"
    ...
    for row_idx, student in enumerate(rows, start=2):
        ...
        status_value = "Expired" if student["expiry_date"] < today else "Active"
    ...
    ws.cell(row=summary_start, column=1, value="Date Range:").font = bold_font
    ws.cell(row=summary_start, column=2, value=range_label).font = normal_font

    ws.cell(row=summary_start + 1, column=1, value="Total Students:").font = bold_font
    ws.cell(row=summary_start + 1, column=2, value=len(rows)).font = normal_font

    ws.cell(row=summary_start + 2, column=1, value="Total Fees Collected:").font = bold_font
    ws.cell(row=summary_start + 2, column=2, value=round(total_fees, 2)).font = normal_font
    ...
    filename = f"students_report_{filename_range}.xlsx"
